...
vec4 updatedPosition() {
    ...
    float movement = 0.f;
    if (u_percent < c_slidingTime) {
        movement = -u_offset * (1.f - u_percent / c_slidingTime);
        position.x = a_position.x + movement;
    } else if (u_percent < c_slidingTime + c_forwardTime) {
        if (a_position.y > u_center.y) {
            position.x += u_resolution.x * c_forwardPercent * (u_percent - c_slidingTime) / c_forwardTime;
        } else {
            position.x += u_resolution.x * c_forwardPercent / 2.f * (u_percent - c_slidingTime) / c_forwardTime;
        }
    } else if (u_percent < c_slidingTime + c_forwardTime + c_backwardTime) {
        float percent = (u_percent - c_slidingTime - c_forwardTime) / c_backwardTime;
        if (a_position.y > u_center.y) {
        ...
        float percent = (u_percent - c_slidingTime - c_forwardTime - c_backwardTime) / c_recoverTime;
        if (a_position.y > u_center.y) {
