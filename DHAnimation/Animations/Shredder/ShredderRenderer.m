...
- (void) startAnimationFromView:(UIView *)fromView toView:(UIView *)toView inContainerView:(UIView *)containerView columnCount:(NSInteger)columnCount duration:(NSTimeInterval)duration direction:(AnimationDirection)direction timingFunction:(NSBKeyframeAnimationFunction)timingFunction completion:(void (^)(void))completion
{
    ...
    for (int i = 0; i < columnCount; i+=2) {
        ShredderPaperPieceSceneMesh *mesh = [[ShredderPaperPieceSceneMesh alloc] initWithScreenWidth:fromView.bounds.size.width screenHeight:fromView.bounds.size.height yResolution:8 totalPieces:columnCount index:i];
    ...
    for (int i = 1; i < columnCount; i+=2) {
        ShredderPaperPieceSceneMesh *mesh = [[ShredderPaperBackPieceSceneMesh alloc] initWithScreenWidth:fromView.bounds.size.width screenHeight:fromView.bounds.size.height yResolution:8 totalPieces:columnCount index:i];
        [self.backMeshes addObject:mesh];
    }
    self.shredderMesh = [[ShredderMesh alloc] initWithScreenWidth:fromView.bounds.size.width screenHeight:fromView.bounds.size.height];
